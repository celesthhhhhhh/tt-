...
def format_proxy(proxy_url):
    """
    Разбирает строку прокси и формирует объект для Playwright.
    ...
    try:
        ...
        # Формируем базовый адрес сервера (протокол + хост + порт)
        # Если порт не указан, urlparse оставит его пустым
        ...
        # Если есть логин и пароль, добавляем их явно
        ...
        # Если разбор не удался, возвращаем как есть (на удачу)
...
def check_ip():
    ...
    # Для requests используем обычный формат строки
